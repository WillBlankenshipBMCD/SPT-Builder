...
def s9000Gen(rxTxUserInput, stationName, revision, emsBaudRate, rtuBaudRate, emsLocalAddress, rtuRemoteAddress):
    ...
    for i in range(mcdPoints.__len__()):
        if mcdPoints[i].get('description') == "":
            numDelete += 1

    if numDelete != 0:
        mcdPoints = mcdPoints[:-numDelete]
    ...
    for i in range(ssPoints.__len__()):
        if ssPoints[i].get('description') == "":
            numDelete += 1

    if numDelete != 0:
        ssPoints = ssPoints[:-numDelete]
    ...
    if mcdPoints.__len__() > 28:
        ...
        for i in range(28):
            if mcdPoints[i].get('description').upper() != "UNDEFINED":
                P = etree.Element("P", Id=str(i), Key = str(mcdBinaryNums[i]), Name=mcdPoints[i].get('description') + "_" + str(sheet.cell_value(i, 12)).strip().replace(" ", "_")
                     .replace("\\", "_").replace("/", "_").replace("-", "_")
                     .replace("&", "").replace("<", "").replace(">", "")
                     .replace('"', "").upper())
                P.text = ""
